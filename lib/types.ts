...
export type Period = { id: string; date: string; total: number; fx: number; cny: number; future: boolean; difference: number; partial?: boolean; archived?: boolean; withdrawn?: number };
...
export type Ledger = { dataKind: 'example' | 'personal'; assets: Asset[]; history: Period[]; withdrawals: Withdrawal[]; fx: number; baselineDate: string; startedAt: string; baselineTotal: number; connections: { bybit: Connection; aster: Connection } };
