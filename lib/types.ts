...
export type Period = { id: string; date: string; total: number; fx: number; cny: number; future: boolean; difference: number; partial?: boolean; archived?: boolean };
export type Connection = { configured: boolean; lastSync: string | null; error: string | null; scope: string; label?: string };
export type Ledger = { dataKind: 'example' | 'personal'; assets: Asset[]; history: Period[]; fx: number; baselineDate: string; startedAt: string; baselineTotal: number; connections: { bybit: Connection; aster: Connection } };
