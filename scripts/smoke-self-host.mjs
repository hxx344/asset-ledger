...
try {
  ...
  const summaryResponse = await request('/api/hub/summary?schemaVersion=2');
  assert.equal(summaryResponse.status, 200);
  assert.equal(summaryResponse.headers.get('cache-control'), 'no-store');
  const summary = await summaryResponse.json();
  assert.equal(summary.schemaVersion, 2);
  assert.equal(summary.data.health.state, 'partial');
  assert.equal(summary.data.metrics.find(metric => metric.key === 'ledger_total').value, ledger.assets.reduce((sum, asset) => sum + (asset.value ?? 0), 0));
  assert.equal('assets' in summary.data, false);
  assert.equal((await request('/api/hub/summary?schemaVersion=3')).status, 400);
  ...
  writeFileSync(join(directory, 'exchange-concurrency'), 'fixture');
  const concurrent = await forceSync();
  assert.equal(concurrent.connections.bybit.error, null, 'Bybit and Aster must overlap after market preparation');
  assert.equal(concurrent.assets.find(asset => asset.mode === 'bybit').value, 75);
  ...
  const legacyDb = new DatabaseSync(join(directory, 'ledger.sqlite'));
  legacyDb.prepare("DELETE FROM settings WHERE owner='owner' AND key='aster-accounts'").run();
  legacyDb.close();
  const legacyLedger = await (await request('/api/ledger')).json();
  assert.equal(legacyLedger.asterAccounts.length, 1);
  assert.equal(legacyLedger.asterAccounts[0].id, 'default');
  const secondWallet = Wallet.createRandom();
  const second = {id:crypto.randomUUID(),name:'Second account',walletAddress:secondWallet.address,privateKey:secondWallet.privateKey,includeSpot:false};
  const fixtures = {[secondWallet.address.toLowerCase()]:{total:250}};
  const saveFixtures=()=>writeFileSync(join(directory,'aster-fixtures.json'),JSON.stringify(fixtures));
  saveFixtures();
  for(const method of ['POST','PATCH','DELETE'])assert.equal((await request('/api/aster-accounts',method,second,{origin:'https://untrusted.example'})).status,403);
  for(const invalid of [{id:'bybit'},{owner:'other'},{name:' '},{privateKey:'invalid'}])assert.equal((await request('/api/aster-accounts','POST',{...second,...invalid})).status,400);
  const addedResponse=await request('/api/aster-accounts','POST',second);
  const addedText=await addedResponse.text();assert.equal(addedResponse.status,200,addedText);
  assert.equal(addedText.includes(secondWallet.privateKey),false);
  const added=JSON.parse(addedText);
  assert.equal(added.asterAccounts.length,2);
  assert.equal(added.assets.find(a=>a.mode==='aster').value,400);
  assert.equal(added.asterAccounts.find(a=>a.id==='default').value,150,'Legacy amount cannot be replaced by aggregate');
  assert.match(added.assets.find(a=>a.mode==='aster').details[0].account,/默认账号/);
  const secondDb=new DatabaseSync(join(directory,'ledger.sqlite'));
  assert.equal(secondDb.prepare("SELECT encrypted FROM connections WHERE owner='owner' AND exchange='aster'").get().encrypted,encrypted,'Adding keeps original credential bytes');
  const secondCipher=secondDb.prepare('SELECT encrypted FROM connections WHERE owner=? AND exchange=?').get('owner','aster:'+second.id).encrypted;
  assert.equal(secondCipher.includes(secondWallet.privateKey),false);
  const secondSealed=JSON.parse(secondCipher);
  await assert.rejects(crypto.subtle.decrypt({name:'AES-GCM',iv:new Uint8Array(secondSealed.iv),additionalData:new TextEncoder().encode('owner:aster')},encryptionKey,new Uint8Array(secondSealed.cipher)),'Ciphertext cannot be moved to another account');
  secondDb.close();
  assert.equal((await request('/api/aster-accounts','POST',{...second,id:crypto.randomUUID(),name:'Duplicate',walletAddress:second.walletAddress.toLowerCase()})).status,400);
  assert.equal((await request('/api/aster-accounts','PATCH',{...second,id:crypto.randomUUID()})).status,400);
  assert.equal((await request('/api/aster-accounts','PATCH',{...second,walletAddress:apiWallet.address})).status,400);
  await stop();await start();
  const restartedMulti=await forceSync();
  assert.equal(restartedMulti.assets.find(a=>a.mode==='aster').value,400);
  const firstTime=restartedMulti.asterAccounts.find(a=>a.id==='default').lastSync;
  fixtures[apiWallet.address.toLowerCase()]={failure:true};fixtures[secondWallet.address.toLowerCase()]={total:300};saveFixtures();
  const partial=await forceSync();
  assert.equal(partial.assets.find(a=>a.mode==='aster').value,450);
  assert.equal(partial.asterAccounts.find(a=>a.id==='default').lastSync,firstTime);
  assert.ok(partial.asterAccounts.find(a=>a.id==='default').error);
  assert.equal(partial.asterAccounts.find(a=>a.id===second.id).error,null);
  assert.equal(partial.history.find(p=>p.id.startsWith('daily-')&&!p.archived).partial,true);
  const partialSummary = await (await request('/api/hub/summary?schemaVersion=2')).json();
  assert.equal(partialSummary.data.health.state, 'partial');
  assert.equal(partialSummary.data.updatedAt, new Date(partial.assets.filter(asset => asset.mode !== 'manual').map(asset => asset.updatedAt).sort((a, b) => Date.parse(a) - Date.parse(b))[0]).toISOString());
  fixtures[secondWallet.address.toLowerCase()]={failure:true};saveFixtures();
  assert.equal((await request('/api/aster-accounts','PATCH',{...second,name:'Rejected update'})).status,400);
  assert.equal((await (await request('/api/ledger')).json()).asterAccounts.find(a=>a.id===second.id).name,second.name);
  fixtures[apiWallet.address.toLowerCase()]={total:125};fixtures[secondWallet.address.toLowerCase()]={total:300};saveFixtures();
  const renamed=await (await request('/api/aster-accounts','PATCH',{...second,name:'Renamed account',includeSpot:true})).json();
  assert.equal(renamed.assets.find(a=>a.mode==='aster').value,475);
  const detached=await (await request('/api/aster-accounts','DELETE',{id:second.id,action:'disconnect'})).json();
  assert.equal(detached.assets.find(a=>a.mode==='aster').value,475);
  assert.equal(detached.asterAccounts.find(a=>a.id===second.id).configured,false);
  assert.equal((await request('/api/aster-accounts','POST',{...second,id:crypto.randomUUID(),name:'Duplicate detached'})).status,400);
  assert.equal((await request('/api/aster-accounts','PATCH',second)).status,200);
  const beforeRemove=await (await request('/api/ledger')).json();
  const originalPeriod=beforeRemove.history.find(p=>p.id==='E1');
  const removed=await (await request('/api/aster-accounts','DELETE',{id:second.id,action:'remove'})).json();
  assert.equal(removed.asterAccounts.length,1);
  assert.equal(removed.assets.find(a=>a.mode==='aster').value,150);
  assert.deepEqual(removed.history.find(p=>p.id==='E1'),originalPeriod);
  assert.equal((await request('/api/aster-accounts','DELETE',{id:'bybit',action:'remove'})).status,400);
  const emptied=await (await request('/api/aster-accounts','DELETE',{id:'default',action:'remove'})).json();
  assert.equal(emptied.assets.find(a=>a.mode==='aster').value,0);
  await stop();await start();
  const emptyRestart=await forceSync();
  assert.equal(emptyRestart.asterAccounts.length,0);
  assert.equal(emptyRestart.assets.find(a=>a.mode==='aster').value,0);
  assert.equal(output.includes(secondWallet.privateKey),false);
  ...
  console.log('Production smoke passed: authentication, import and backup, edits/history, Aster multi-account upgrade/CRUD/encryption/isolation/partial failures, automatic FX/failure preservation, restart synchronization, withdrawal CRUD/persistence and login throttling.');
