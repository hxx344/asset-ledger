...
**升级再次执行同一条命令。** 未指定端口和 `--local` 时保留原端口及监听地址，所以旧版公网监听需要显式加 `--local` 切换；保留密码、加密密钥、资产和历史。版本、环境和配置未变且服务健康时跳过安装、构建、备份和重启；仅改端口时复用构建，代码更新且依赖未变时复用依赖的独立副本。先构建新版本，再停止旧服务并备份数据库；启动检查失败时恢复旧版本、端口和数据库。
...
正常升级也会先执行清理。清理后仍不足时，根据提示释放其他文件或扩容，再重复升级命令。
